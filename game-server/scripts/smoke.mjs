...
const join = (query = '') => new Promise((resolve, reject) => {
  const socket = new WebSocket(base.replace(/^http/, 'ws') + '/ws?build=meadow-network-1' + query.replace('?', '&'));
  sockets.push(socket);
  const timer = setTimeout(() => { socket.close(); reject(new Error('Join timed out')); }, 5000);
  socket.addEventListener('message', event => { clearTimeout(timer); resolve({ socket, welcome: JSON.parse(event.data) }); }, { once: true });
...
try {
  ...
  const results = await Promise.allSettled(Array.from({ length: 20 }, (_, i) => join('?room=alternate-' + i)));
  ...
  assert.deepEqual(await get('/room'), { room: 'main', players: 14, capacity: 14, full: true, protocolVersion: 2 });
  assert.deepEqual(await get('/players'),{players:14});
  await assert.rejects(join(), /Join rejected/);
  accepted[0].socket.close(1000, 'Leaving');
  await waitForCount(13);
  assert.equal((await join()).welcome.players.length, 14);
