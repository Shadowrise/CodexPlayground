...
import {SkyTrail} from '../src/sky-trail';import {SKY_PLATFORMS as P,SKY_TRAIL_SITE as S,SKY_CHECKPOINTS,SKY_COLORS,rainbowHeight} from '../src/sky-trail-layout';import {CharacterController} from '../src/controller';import {scoreOf} from '../src/score';import {sceneryClearance} from '../src/landmarks';
...
test('rainbow has seven bands and route steps are reachable by a normal single jump',()=>{assert.equal(SKY_COLORS.length,7);for(let i=1;i<P.length;i++){assert(P[i].y-P[i-1].y<=1.45);if(i!==15)assert(Math.hypot(P[i].x-P[i-1].x,P[i].z-P[i-1].z)<6);}assert.equal(rainbowHeight(P[14].x),P[14].y);assert(Math.abs(rainbowHeight(8.5)!-P[15].y)<1e-8);assert(!sceneryClearance(S.x,S.z));});
test('jump through all platforms, walk the rainbow, collect once and return via leaves',async()=>{
 const c=await player(),trail=new SkyTrail();c.actor.position.copy(trail.entry);assert(trail.start(c));for(let i=0;i<180;i++)trail.update(1/60,c);assert(!trail.active);assert(c.actor.position.distanceTo(pos(0))<.01);
 for(let step=1;step<P.length;step++){
  const start=c.actor.position.clone(),end=pos(step);
  if(step===15){for(let i=1;i<=150;i++){const previous=c.actor.position.clone();c.actor.position.lerpVectors(start,end,i/150);c.actor.position.y=previous.y;trail.apply(c,previous,1/60);}assert(Math.abs(c.actor.position.y-end.y)<.01);continue;}
 ...
 assert(trail.start(c));for(let i=0;i<250;i++)trail.update(1/60,c);assert(!trail.active);assert(c.actor.position.distanceTo(trail.landing)<.01);
});
test('falling off keeps checkpoints and lower trampoline returns to the last reached one',async()=>{const c=await player(),trail=new SkyTrail();c.skyCheckpoint=2;c.actor.position.copy(trail.entry);assert(trail.start(c));for(let i=0;i<180;i++)trail.update(1/60,c);assert(c.actor.position.distanceTo(pos(SKY_CHECKPOINTS[1]))<.01);
 ...
 c.actor.position.copy(trail.entry);assert(trail.start(c));for(let i=0;i<180;i++)trail.update(1/60,c);assert(c.actor.position.distanceTo(pos(SKY_CHECKPOINTS[1]))<.01);
});
