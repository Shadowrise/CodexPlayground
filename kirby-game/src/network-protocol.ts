...
export type ServerMessage=Welcome|{type:'frame';id:string;actor?:ActorState;world?:WorldState}|{type:'room';room:RoomState}|{type:'left';id:string}|{type:'hit';id:string;actor:ActorState}|{type:'lock';key:string;ok:boolean}|{type:'star'}|{type:'error';message:string};
...
export function validActor(v:unknown):v is ActorState{
 ...
 const length=kind==='cart'?4:kind==='balloon'?16:kind==='bug'?9:kind==='tree'?1:0;
