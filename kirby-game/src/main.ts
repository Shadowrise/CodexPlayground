...
renderer.setAnimationLoop((time: number) => {
  ...
  if (character) {
    ...
    if(!coaster.riding && !treehouse.active && !balloons.riding){
  ...
  else {coaster.update(dt);treehouse.update(dt);balloons.update(dt);maze.update(dt);}
