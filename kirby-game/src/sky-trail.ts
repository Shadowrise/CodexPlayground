...
export class SkyTrail{
 readonly group=new T.Group();readonly entry=new T.Vector3(SITE.x,0,SITE.z+17);
 ...
 private rider?:CharacterController;private travel?:{from:T.Vector3;to:T.Vector3;elapsed:number;duration:number;leaves:boolean;landed:boolean};
 ...
 private trampoline(parent:T.Group){
  ...
  const pad=new T.Mesh(new T.CylinderGeometry(1.4,1.4,.08,40),new T.MeshStandardMaterial({color:'#6056a1',roughness:.8}));pad.position.y=.52;parent.add(pad);
 ...
 update(dt:number,c?:CharacterController){
  ...
  t.elapsed+=dt;rider.mixer.update(dt);const u=Math.min(1,t.elapsed/t.duration),blend=u*u*(3-2*u);rider.actor.position.lerpVectors(t.from,t.to,blend);rider.actor.position.y+=Math.sin(u*Math.PI)*(t.leaves?9:Math.max(3,(t.to.y-t.from.y)*.35));
  ...
  if(u===1){if(!t.landed){t.landed=true;if(t.leaves){this.leaves.burst();this.sound('leaves');}else this.sound('checkpoint');}rider.actor.position.copy(t.to);if(!t.leaves||t.elapsed>t.duration+.65){rider.setActivity('Idle');rider.surfaceY=this.base=t.to.y;this.inCourse=this.contains(t.to);this.rider=undefined;this.travel=undefined;}}
 }
